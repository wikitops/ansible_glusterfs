...
$os                     = "ubuntu"
$num_instances          = 3
$num_instances_server   = 2
$num_instances_client   = 1
$instance_name_prefix   = "glusterfs"
$vm_memory              = 2048
$vm_cpus                = 1
$subnet                 = "10.0.3.6" # For 10.0.3.18X
$box                    = SUPPORTED_OS[$os][:box]

# Ansible provisioner
$playbook               = "glusterfs.yml"
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "glusterfs_server" => ["#{$instance_name_prefix}0[1:#{$num_instances_server}]"],
            "glusterfs_client" => ["#{$instance_name_prefix}0[1:#{$num_instances_client}]"]
          }
        end
      end
    end
